		...
		<div class="mx-auto max-w-[90rem] px-5 py-4 sm:px-8 lg:px-12">
			<form id="filters-form" class="pv-gallery-filters" onsubmit="return false;">
				<label class="pv-gallery-search">
					<span class="sr-only"><?php esc_html_e( 'Rechercher', 'photovault' ); ?></span>
				...
				<label><span class="sr-only"><?php esc_html_e( 'Collection', 'photovault' ); ?></span><select id="filter-folder" name="folder"><option value=""><?php esc_html_e( 'Toutes les collections', 'photovault' ); ?></option><?php foreach ( $folders as $folder ) : ?><option value="<?php echo esc_attr( $folder->term_id ); ?>"><?php echo esc_html( $folder->name ); ?></option><?php endforeach; ?></select></label>
				<label><span class="sr-only"><?php esc_html_e( 'Categorie', 'photovault' ); ?></span><select id="filter-category" name="category"><option value=""><?php esc_html_e( 'Toutes les categories', 'photovault' ); ?></option><?php foreach ( $categories as $category ) : ?><option value="<?php echo esc_attr( $category->term_id ); ?>"><?php echo esc_html( $category->name ); ?></option><?php endforeach; ?></select></label>
				<label><span class="sr-only"><?php esc_html_e( 'Protection', 'photovault' ); ?></span><select id="filter-protected" name="protected"><option value=""><?php esc_html_e( 'Tous les acces', 'photovault' ); ?></option><option value="1"><?php esc_html_e( 'Oeuvres protegees', 'photovault' ); ?></option><option value="0"><?php esc_html_e( 'Oeuvres ouvertes', 'photovault' ); ?></option></select></label>
				<label><span class="sr-only"><?php esc_html_e( 'Tri', 'photovault' ); ?></span><select id="filter-orderby" name="orderby"><option value="date_desc"><?php esc_html_e( 'Plus recentes', 'photovault' ); ?></option><option value="date_asc"><?php esc_html_e( 'Plus anciennes', 'photovault' ); ?></option><option value="alphabetical"><?php esc_html_e( 'Titre A-Z', 'photovault' ); ?></option></select></label>
				<button id="reset-filters" class="pv-gallery-reset" type="reset" title="<?php esc_attr_e( 'Reinitialiser les filtres', 'photovault' ); ?>" aria-label="<?php esc_attr_e( 'Reinitialiser les filtres', 'photovault' ); ?>"><svg aria-hidden="true" viewBox="0 0 24 24" fill="none" stroke="currentColor"><path d="M3 12a9 9 0 1 0 3-6.7L3 8"/><path d="M3 3v5h5"/></svg></button>
			</form>
		</div>
	</section>
