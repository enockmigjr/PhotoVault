...
$identity_account_endpoint = rest_url( 'identity-security-kit/v1/account' );
$preferences_endpoint      = rest_url( 'photovault/v1/account/preferences' );
		...
		<div class="mx-auto max-w-5xl">
			...
			<?php if ( ! $email_verified ) : ?>
				<section class="mb-10 flex flex-col gap-4 border-l-4 border-amber-300 bg-amber-300/10 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
					...
					<form action="<?php echo esc_url( identity_security_kit_get_account_action_url() ); ?>" method="post" data-pv-async-form data-profile-async data-profile-action="resend_verification" data-pv-endpoint="<?php echo esc_url( $identity_account_endpoint ); ?>"><input type="hidden" name="account_action" value="resend_verification"><input type="hidden" name="action" value="identity_security_kit_resend_email_verification"><?php wp_nonce_field( 'identity_security_kit_resend_email_verification' ); ?><button class="text-sm font-bold text-amber-200 underline underline-offset-4" type="submit"><?php esc_html_e( 'Renvoyer le lien', 'photovault' ); ?></button></form>
				</section>
			<?php endif; ?>
			...
			<section class="border-b border-white/10 pb-10" aria-labelledby="profile-identity-title">
				<div class="flex flex-col gap-7 sm:flex-row sm:items-center">
					<div class="relative shrink-0"><img class="h-28 w-28 rounded-full object-cover ring-1 ring-white/20" data-profile-avatar-image src="<?php echo esc_url( $avatar_url ); ?>" alt="<?php echo esc_attr( $current_user->display_name ); ?>"><button class="absolute -bottom-1 -right-1 inline-flex h-10 w-10 items-center justify-center rounded-full border border-white/20 bg-[#22201d] text-amber-200 shadow-xl" type="button" data-profile-open="profile-avatar-dialog" aria-label="<?php esc_attr_e( 'Modifier la photo', 'photovault' ); ?>"><svg class="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8" d="M4 7h3l2-2h6l2 2h3v12H4V7z"/><circle cx="12" cy="13" r="3"/></svg></button></div>
					<div class="min-w-0 flex-1"><h2 id="profile-identity-title" class="text-2xl font-bold" data-profile-display-name><?php echo esc_html( $current_user->display_name ?: $current_user->user_login ); ?></h2><p class="mt-2 break-all text-sm text-gray-400"><?php echo esc_html( $current_user->user_email ); ?></p><p class="mt-3 max-w-2xl text-sm leading-6 text-gray-300" data-profile-bio data-empty-label="<?php esc_attr_e( 'Aucune biographie renseignee.', 'photovault' ); ?>"><?php echo esc_html( $current_user->description ?: __( 'Aucune biographie renseignee.', 'photovault' ) ); ?></p></div>
			...
			<section class="py-10" aria-labelledby="profile-contact-title">
				...
				<div class="divide-y divide-white/10 border-y border-white/10">
					...
					<div class="grid gap-4 py-6 sm:grid-cols-[180px,1fr,auto] sm:items-center"><span class="text-sm font-semibold text-gray-400"><?php esc_html_e( 'Telephone', 'photovault' ); ?></span><div><strong class="text-sm" data-profile-phone data-empty-label="<?php esc_attr_e( 'Non renseigne', 'photovault' ); ?>"><?php echo esc_html( $phone ?: __( 'Non renseigne', 'photovault' ) ); ?></strong><span class="ml-2 text-xs font-bold <?php echo $phone_verified ? 'text-emerald-300' : 'text-amber-200'; ?>" data-profile-phone-status><?php echo $phone ? esc_html( $phone_verified ? __( 'Verifie', 'photovault' ) : __( 'A verifier', 'photovault' ) ) : ''; ?></span></div><button class="text-left text-sm font-bold text-amber-200" type="button" data-profile-open="profile-phone-dialog"><?php echo esc_html( $phone ? __( 'Modifier', 'photovault' ) : __( 'Ajouter', 'photovault' ) ); ?></button></div>
			...
			<section class="border-t border-white/10 py-10" aria-labelledby="profile-preferences-title">
				<div class="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between"><div><p class="text-xs font-bold uppercase tracking-[0.2em] text-gray-500"><?php esc_html_e( 'Experience', 'photovault' ); ?></p><h2 id="profile-preferences-title" class="mt-2 text-2xl font-bold"><?php esc_html_e( 'Preferences personnelles', 'photovault' ); ?></h2><p class="mt-3 text-sm text-gray-400" data-profile-preferences-summary><?php echo esc_html( sprintf( __( 'Galerie %1$s - animations %2$s - ouverture sur %3$s', 'photovault' ), 'compact' === $gallery_density ? __( 'compacte', 'photovault' ) : __( 'editoriale', 'photovault' ), '1' === $reduce_motion ? __( 'reduites', 'photovault' ) : __( 'actives', 'photovault' ), $dashboard_landing_label ) ); ?></p></div><button class="text-left text-sm font-bold text-amber-200" type="button" data-profile-open="profile-preferences-dialog"><?php esc_html_e( 'Personnaliser', 'photovault' ); ?></button></div>
			</section>
