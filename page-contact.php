...
<main class="min-h-screen bg-[#0d0c0b] text-gray-100">
	...
	<section class="mx-auto grid max-w-[90rem] gap-12 px-5 py-16 sm:px-8 lg:grid-cols-12 lg:px-12 lg:py-24" aria-labelledby="contact-form-title">
		...
		<div class="lg:col-span-7 lg:col-start-6">
			...
			<form class="pv-public-form mt-8" action="<?php echo esc_url( get_permalink() ); ?>" method="post" data-pv-async-form data-pv-endpoint="<?php echo esc_url( rest_url( 'photovault/v1/application/contact' ) ); ?>" data-pv-success="reset">
				...
				<label data-pv-access-collection <?php echo 'access' !== $request_type ? 'hidden' : ''; ?>><span><?php esc_html_e( 'Collection protégée', 'photovault' ); ?></span><select name="collection_name" <?php echo 'access' === $request_type ? 'required' : ''; ?>><option value=""><?php esc_html_e( 'Sélectionner une collection', 'photovault' ); ?></option><?php foreach ( $collections as $folder ) : ?><option value="<?php echo esc_attr( $folder->name ); ?>" <?php selected( $collection, $folder->name ); ?>><?php echo esc_html( $folder->name ); ?></option><?php endforeach; ?></select></label>
