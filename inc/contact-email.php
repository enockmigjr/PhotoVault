<?php
/**
 * Contact notification composition for the public theme form.
 *
 * @package PhotoVault
 */
...
/** Send one professional studio notification without spoofing the visitor as sender. */
function photovault_send_contact_notification( $data ) {
	...
	if ( function_exists( 'photovault_send_transactional_email' ) ) {
		...
		$acknowledgement = array(
			...
			'details'      => array( sprintf( __( 'Type : %s', 'photovault' ), $types[ $request_type ] ), sprintf( __( 'Sujet : %s', 'photovault' ), $subject ) ),
			'action_url'   => home_url( '/' ),
		...
		$visitor_sent = photovault_send_transactional_email( $email, __( '[PhotoVault] Votre message est bien recu', 'photovault' ), $acknowledgement, $recipient );

		return $studio_sent && $visitor_sent;
	}

	$body = implode( "\n\n", array_filter( array_merge( array( $content['title'], $content['greeting'], $content['intro'] ), $content['details'] ) ) );

	$studio_sent = wp_mail( $recipient, $mail_subject, $body, array( 'Reply-To: ' . $email ) );
	$visitor_body = implode( "\n\n", array( __( 'Message bien recu', 'photovault' ), sprintf( __( 'Bonjour %s,', 'photovault' ), $name ), __( 'Votre message a bien ete transmis au studio.', 'photovault' ), sprintf( __( 'Sujet : %s', 'photovault' ), $subject ) ) );

	return wp_mail( $email, __( '[PhotoVault] Votre message est bien recu', 'photovault' ), $visitor_body, array( 'Reply-To: ' . $recipient ) ) && $studio_sent;
}
