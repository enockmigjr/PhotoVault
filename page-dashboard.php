				...
				<section class="grid gap-10 lg:grid-cols-[1fr,1.2fr]">
					...
					<div class="border-y border-white/10 py-6">
						...
						<?php elseif ( function_exists( 'newsletter_campaign_kit_handle_subscribe' ) ) : ?>
							...
							<?php $dashboard_topics = function_exists( 'newsletter_campaign_kit_get_topics' ) ? newsletter_campaign_kit_get_topics( 20, 0 ) : array(); ?>
							<form action="<?php echo esc_url( home_url( '/newsletter/subscribe/' ) ); ?>" method="post" class="space-y-5" data-pv-async-form data-pv-newsletter-steps data-pv-endpoint="<?php echo esc_url( rest_url( 'newsletter-campaign-kit/v1/subscriptions' ) ); ?>" data-pv-success="reset">
								<input type="hidden" name="action" value="newsletter_campaign_kit_subscribe">
								<input type="hidden" name="newsletter_source" value="dashboard">
								<input type="hidden" name="newsletter_return_url" value="<?php echo esc_url( add_query_arg( 'section', 'newsletter', home_url( '/dashboard/' ) ) ); ?>">
								<?php wp_nonce_field( 'newsletter_campaign_kit_subscribe', 'newsletter_campaign_kit_nonce' ); ?>
								<label class="block text-sm font-semibold" for="dashboard-newsletter-email"><?php esc_html_e( 'Adresse e-mail', 'photovault' ); ?></label>
								<input id="dashboard-newsletter-email" class="w-full rounded-md border border-white/15 bg-black/30 px-4 py-3 text-gray-300" name="newsletter_email" type="email" readonly value="<?php echo esc_attr( $current_user->user_email ); ?>">
								<label class="flex items-start gap-3 text-sm text-gray-400"><input class="mt-1" type="checkbox" name="newsletter_consent" value="1" required><span><?php esc_html_e( 'J accepte de recevoir les lettres editoriales et je pourrai me desinscrire a tout moment.', 'photovault' ); ?></span></label>
								<?php if ( $dashboard_topics ) : ?>
									<fieldset hidden data-pv-newsletter-topics>
										<legend class="mb-3 text-sm font-semibold"><?php esc_html_e( 'Choisissez les thématiques souhaitées', 'photovault' ); ?></legend>
										<div class="grid gap-3 sm:grid-cols-2"><?php foreach ( $dashboard_topics as $topic ) : ?><label class="flex items-start gap-3 border border-white/10 px-4 py-3 text-sm text-gray-300"><input class="mt-1" type="checkbox" name="newsletter_topic_ids[]" value="<?php echo esc_attr( $topic['id'] ); ?>"><span><strong class="block text-white"><?php echo esc_html( $topic['name'] ); ?></strong><?php if ( $topic['description'] ) : ?><small class="mt-1 block text-gray-500"><?php echo esc_html( $topic['description'] ); ?></small><?php endif; ?></span></label><?php endforeach; ?></div>
									</fieldset>
								<?php endif; ?>
								<button class="rounded-md bg-amber-300 px-5 py-3 text-sm font-black text-black" type="submit" data-pv-newsletter-submit><?php echo esc_html( $subscriber && 'pending' === $subscriber['status'] ? __( 'Renvoyer la confirmation', 'photovault' ) : __( 'Continuer', 'photovault' ) ); ?></button>
							</form>
