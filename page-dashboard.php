...
$sections     = array( 'overview', 'favorites', 'downloads', 'access', 'newsletter', 'security' );
if ( $is_manager ) {
	$sections[] = 'analytics';
}
		...
		<div class="mx-auto max-w-7xl">
			...
			<?php if ( 'overview' === $section ) : ?>
				<section aria-labelledby="dashboard-summary-title">
					...
					<div class="grid gap-px overflow-hidden rounded-md border border-white/10 bg-white/10 sm:grid-cols-2 xl:grid-cols-4">
						<?php
						$summary = array(
			...
			<?php elseif ( 'newsletter' === $section ) : ?>
				<section class="grid gap-10 lg:grid-cols-[1fr,1.2fr]">
					<div><p class="text-xs font-bold uppercase tracking-[0.2em] text-gray-500"><?php esc_html_e( 'Abonnement', 'photovault' ); ?></p><h2 class="mt-2 text-2xl font-bold"><?php echo esc_html( $subscriber && 'subscribed' === $subscriber['status'] ? __( 'Vous recevez les carnets PhotoVault', 'photovault' ) : __( 'Restez au courant des nouvelles archives', 'photovault' ) ); ?></h2><p class="mt-4 text-sm leading-7 text-gray-400"><?php esc_html_e( 'Les preferences thematiques permettent de recevoir uniquement les lettres correspondant aux sujets choisis.', 'photovault' ); ?></p></div>
